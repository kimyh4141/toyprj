...
function getCurrentLocation() {
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      ...
      function () {
        alert(
          'Geolocation을 지원하지 않으므로, 기본 위치를 중심으로 검색합니다.',
        );
        searchPlaces();
      },
    );
    ...
    alert(
      '이 브라우저에서는 Geolocation이 지원되지 않습니다. 기본 위치를 중심으로 검색합니다.',
    );
    searchPlaces();
  }
}
...
function addRedMarker(position) {
  var imageSrc =
      'https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/red_b.png', // 마커 이미지 url
